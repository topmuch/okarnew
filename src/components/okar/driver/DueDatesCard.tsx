...
import { Shield, Droplets, Calendar, Clock, ArrowRight } from 'lucide-react'
...
interface DueDatesCardProps {
  ...
  onCompareInsurance?: () => void
}

export function DueDatesCard({ insurance, technicalControl, oilChange, onCompareInsurance }: DueDatesCardProps) {
  ...
  // Check if insurance needs attention
  const showInsuranceCTA = (insurance.status === 'expiring_soon' || insurance.status === 'expired') && onCompareInsurance
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <div className={`p-4 rounded-2xl border bg-gradient-to-br ${getStatusColor(insurance.status)}`}>
            ...
            {showInsuranceCTA && (
              <Button
                size="sm"
                className="w-full mt-3 bg-gradient-to-r from-orange-500 to-pink-500 hover:from-orange-600 hover:to-pink-600 text-white rounded-xl"
                onClick={onCompareInsurance}
              >
