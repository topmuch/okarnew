...
export async function POST(request: NextRequest) {
  try {
    ...
    // Mettre à jour la prochaine vidange si c'est une vidange
    if (type === 'oil_change') {
      await db.vehicle.update({
        where: { id: vehicleId },
        data: {
          nextOilChangeMileage: parseInt(mileage) + 5000,
          nextOilChangeDate: new Date(Date.now() + 180 * 24 * 60 * 60 * 1000), // 6 mois
        }
      })
    }

    // Récupérer les infos du véhicule pour WhatsApp
    ...
    return NextResponse.json({ 
      success: true, 
      data: intervention,
      // Include WhatsApp info for client notification
      notification: {
        phone: vehicleInfo?.owner?.phone,
        ownerName: vehicleInfo?.owner?.name,
        vehiclePlate: vehicleInfo?.plateNumber,
        qrCode: vehicleInfo?.qrCode?.code
      }
    })
  } catch (error) {
