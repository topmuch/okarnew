...
async function seedQRCodes(garages: { id: string; businessName: string; userId: string }[]) {
  ...
  if (existingCount > 0) {
    ...
    // Vérifier et créer les QR codes de test s'ils n'existent pas
    ...
    for (const code of testCodes) {
      ...
      if (!existing) {
        ...
        console.log(`   ✅ QR code de test créé: ${code}`)
        totalCreated++
      }
    }
    return existingCount + totalCreated
  }
  
  // Créer les QR codes de test en premier
  console.log('   📦 Création des QR codes de test...')
  const testCodes = ['OKAR-TEST001', 'OKAR-TEST002', 'OKAR-TEST003']
  for (const code of testCodes) {
    await prisma.qRCode.create({
      data: {
